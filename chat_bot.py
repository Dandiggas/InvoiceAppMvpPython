...
def generate_invoice(invoice_data: Invoice) -> InvoiceResponse:
    ...
    try:
        ...
        services = [(invoice_data.service_details, invoice_data.service_price)]
        
        # Calculate total
        total_cost = str(invoice_data.service_price)
        
        # Generate PDF name
        pdf_name = f"invoice_{invoice_data.invoice_number}.pdf"
        ...
        return InvoiceResponse(
            success=True, 
            message=f"Invoice generated successfully. Please review the PDF at {pdf_path} before sending.", 
            pdf_path=pdf_path
        )
...
@tool
def send_invoice_email(pdf_path: str, client_email: str, custom_message: str = None) -> EmailResponse:
    ...
    try:
        ...
        return EmailResponse(
            success=True,
            message=f"Invoice successfully sent to {client_email}"
        )
    except Exception as e:
        return EmailResponse(
            success=False,
...
def chatbot(state: State):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        You can also send invoices via email when asked using the send_invoice_email tool.
