...
def main():
    while True:
        ...
        date_input = input(
            f"Enter date (or press enter for today's date {formatted_date}): "
        )
        if not date_input:
        ...
        while True:
            service_desc = input(
                "Enter service description(or press enter to finish): "
            )
            if not service_desc:
                break
        ...
        invoice_check = input(
            "Would you like me to email the invoice to the client, yes or no? "
        )
        invoice_check.strip()
